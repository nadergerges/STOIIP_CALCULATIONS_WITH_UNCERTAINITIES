...
# Streamlit app title
st.title("STOIIP Calculator (Acres) with Monte Carlo Simulation (PyVista)")
...
# 2) Histogram of STOIIP distribution with lines for P10, P50, P90
...
# ----------------------
st.header("3D Reservoir Volume Visualization (PyVista)")
...
# We'll subdivide the reservoir into 200x200 m cells in X,Y, and 200 m in Z
cell_size_xy = 200.0
cell_size_z = 200.0
...
if Nx < 1:
    Nx = 1
Ny = Nx  # keep it square in plan view
...
def build_3d_wireframe_pyvista(Nx, Ny, Nz, dx, dy, dz):
    """
    Returns a PyVista Plotter with lines making up a wireframe grid of Nx x Ny x Nz cells
    in X, Y, Z directions.
    """
    plotter = pv.Plotter(off_screen=True)
    
    # We'll add lines for each direction (X, Y, Z).
    # We do a loop similar to what we did with Plotly, but now using plotter.add_lines().
    ...
    for j in range(Ny+1):
        for k in range(Nz+1):
            ...
            points = np.array([[x0, y0, z0], [x1, y1, z1]])
            plotter.add_lines(points, color='black', width=1)
